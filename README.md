...
- Scanner 페이지(운영형 2-pane)
  ...
  - 최소 프레임: Scanner `minWidth=1260`, `minHeight=760`, 좌측 pane `minWidth=500`, 우측 pane `minWidth=700`
...
7. 창 최소 크기 근처에서도 좌측/우측 pane 하단 라인이 크게 어긋나지 않고, 좌측 목록만 내부 스크롤되는지 확인
8. 좌측 리스트에서 종목명이 세로로 꺾이지 않고 1줄 기준으로 자연스럽게 truncation 되는지 확인
9. `Logs` 페이지에서 좌측 항목 클릭 시 우측 상세 패널이 즉시 갱신되는지 확인
10. snapshot/WS 데이터 수신 시 좌측 목록과 선택 상세가 함께 갱신되는지 확인
