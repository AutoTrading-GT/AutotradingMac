...
- Settings 페이지(운영형 2x2 패널)
  ...
  - `백업 디렉토리`는 Settings에서 직접 저장 가능하며, 서버 파일시스템 경로를 편집한다.
  - 폴더 선택 버튼은 경로 입력 보조용이며, 실제로 적용되는 경로는 서버가 정규화해 돌려준 절대경로다.
...
- Stategy 페이지(사이드바 독립 화면)
  ...
  - 레이아웃:
    - 상단: 현재 전략 요약
    - 본문: `Basic Strategy`(진입/청산/리스크) 우선 노출
    - 하단: `Advanced Settings`(기존 Scanner/Signal/Risk 상세 튜닝, 기본 접힘)
  ...
  - 정보 구조/정렬 원칙:
    ...
    - `Basic Strategy`는 각 전략을 작은 카드 묶음이 아니라 하나의 긴 가로 패널로 정리한다
      - 진입: `후보 선정 방식` / `관찰 후보 수` / `진입 신호`
      - 청산: `익절·손절` / `보유 시간 제한` / `장마감 청산`
      - 리스크: `최대 손실 한도` / `포지션 크기` / `거래 제한`
  ...
  - Basic 편집 항목:
    ...
    - 리스크: 최대 손실 한도, 포지션 크기(전체 자산 대비 %), 일일 거래 제한 사용/최대 횟수, 동시 보유 제한
  - 입력 컨트롤 규칙:
