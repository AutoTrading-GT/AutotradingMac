...
- 본문 2컬럼
  - 좌측: 스캔종목, 보유종목
  - 우측: 매매신호, 미체결주문, 최근 매매/이벤트
- Dashboard 요약 정책(결과 중심)
  - `최근 매매/이벤트`는 Logs와 동일 대표 이벤트 규칙으로 압축하여 주문/체결/청산 중복을 줄임
  - `매매신호`는 실행 관련 신호만 표시
    - 포함: 매수/매도/진입/청산 계열 신호, `risk.approved`
    - 제외: 관망(`watch`/`rank_maintained`), 보류/차단(`risk.blocked`) 계열
